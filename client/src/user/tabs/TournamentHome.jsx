        ...
        <>
            <section className='col-span-5 h-full min-h-screen flex flex-col bg-card'>
                ...
                <div className="flex gap-10 p-10">
                    ...
                    {
                        tournamentData.filter(tournament => tournament.status != 'Draft').map((item, index) => {
                            const currentOrgData = organizationData?.find(organizatoin => organizatoin._id == item.orgID)
                            return (

                                <div key={'tournament' + index} onClick={() => navigate(`/tournaments/${item._id}`)} className="p-5 bg-accent grid grid-cols-2 gap-5 cursor-pointer hover:bg-[#5a5a5a] duration-500">
