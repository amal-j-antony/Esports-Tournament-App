        ...
        <>
            <main className="bg-black/30 backdrop-blur grid justify-center items-center fixed inset-0">
                <section className="grid gap-5 p-5 bg-card rounded-2xl min-w-100">
                    ...
                    <input {...register(`stageInfo.${index}.stageName`)} type="text" className={inputStyle} />
                    <div className="flex justify-end gap-5">
                        <button onClick={() => {
                            resetField(`stageInfo.${index}.stageName`)
                            setStageNameDialog(false)}} className="py-1 px-3 bg-accent rounded-xl cursor-pointer">Cancel</button>
                        <button onClick={()=>setStageNameDialog(false)} className="py-1 px-3 bg-zinc-500 rounded-xl cursor-pointer">Proceed</button>
                    </div>
                </section>
            </main>
        </>
